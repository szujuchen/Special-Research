...
class IMG_DATA(torch.utils.data.Dataset):
    ...
    def _construct_db(self):
        ...
        self._db = []
        with open(os.path.join(self._data_path, 'retrieval.pkl'), 'rb') as fin:
            gnd = pkl.load(fin)
            if self._split == 'query':
                for i in range(len(gnd["qimlist"])):
                    im_fn = gnd["qimlist"][i]
                    im_path = os.path.join(self._data_path, "images", im_fn)
...
class TEST_IMG(torch.utils.data.Dataset):
    """Common dataset."""
    def __init__(self, data_path, transform):
        assert os.path.exists(data_path), "Data path '{}' not found".format(data_path)
        self._data_path, self._transform = data_path, transform
        self._construct_db()
    ...
    def _construct_db(self):
        ...
        # Compile the split data path
        self._db = []
        self._db.append({"im_path": self._data_path})
